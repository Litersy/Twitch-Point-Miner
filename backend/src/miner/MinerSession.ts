...
export class MinerSession {
  ...
  private async onPoints(payload: any): Promise<void> {
    if (payload?.type === 'points-earned') {
      ...
      if (!gain) return;
      // Twitch's authoritative running balance. If it's absent we must NOT invent
      ...
      // Only snapshot the balance when Twitch gave us a real one.
      if (balance !== null) {
      ...
      // Don't assert success here — the real "+N (CLAIM)" line is logged from the
      // points-earned event only after Twitch actually credits the bonus. We only
      // surface a claim that Twitch rejected (already claimed / expired).
      if (!ok) {
        await this.writeLog('warn', 'points', `bonus claim rejected on ${streamer.login} (already claimed or expired)`);
      }
    }
  }
    ...
    for (const s of this.streamers) {
      try {
        ...
        if (ctx.availableClaimId && !sleeping) {
          ...
          // Success shows up as the authoritative "+N (CLAIM)" points-earned line;
          // only report a claim Twitch actually rejected.
          if (!ok) {
            await this.writeLog('warn', 'points', `initial bonus claim rejected on ${s.login} (already claimed or expired)`);
          }
        }
      } catch (err: any) {
